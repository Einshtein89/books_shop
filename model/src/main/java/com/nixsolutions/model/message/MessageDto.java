...
public final class MessageDto
{
  ...
  public Message convertToMessage()
  {
    ...
    return new Message(UUID.randomUUID().toString(), userId, ordersCount, orderId, LocalDate.now().format(formatter));
  }
  ...
  public static Message convertToMessage(GenericRecord record)
  {
    return new Message(convertFromUtf8((Utf8) record.get("uuid")), convertFromUtf8((Utf8) record.get("userId")),
        (int)record.get("ordersCount"),  convertFromUtf8((Utf8) record.get("orderId")),
        convertFromUtf8((Utf8) record.get("time")));
  }
